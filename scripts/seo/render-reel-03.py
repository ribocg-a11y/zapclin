#!/usr/bin/env python3
"""ZapClin Reel #03 — Calhau / Golden Shopping (9:16 20s)."""
...
OUT_DIR = Path("/opt/cursor/artifacts/zapclin-reel-03")
...
def render_frame(i: int) -> Image.Image:
    ...
    if 2.9 < t < 7.2:
        ...
        lines = ["Quiosque 01", "Golden Shopping", "CALHAU"]
        ...
        for idx, (txt, col) in enumerate(zip(lines, colors)):
            ...
            y += sizes[idx] + 18
        pill = "Av. dos Holandeses, 200 · São Luís"
        fp = font(FONT_REG, 34)
        if local_t > 1.1:
...
def main():
    out_mp4 = OUT_DIR / "zapclin-reel-03-8min-9x16.mp4"
    out_mp4_repo = REPO_OUT / "zapclin-reel-03-8min-9x16.mp4"
    caption_path = OUT_DIR / "zapclin-reel-03-legenda.txt"
    caption_repo = REPO_OUT / "zapclin-reel-03-legenda.txt"

    caption = """Higienização de capacete no Calhau — Golden Shopping Calhau, Quiosque 01.

A partir de R$ 15 · ~8 min. Deixa o capacete e segue no shopping.
Processo profissional ZapClin · São Luís / MA.

📍 Av. dos Holandeses, 200 · Calhau
🌐 https://www.zapclinslz.com/
WhatsApp: 98 98147-9616

Salva este Reel e marca alguém que passa no Golden 👇
Link na bio.

#ZapClin #HigienizacaoDeCapacete #SaoLuis #Calhau #GoldenShoppingCalhau #MotoboySLZ #CapaceteMoto #Maranhao
"""
    ...
    poster.save(OUT_DIR / "zapclin-reel-03-poster.jpg", quality=92)
    poster.save(REPO_OUT / "zapclin-reel-03-poster.jpg", quality=92)
