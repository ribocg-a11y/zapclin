#!/usr/bin/env python3
"""ZapClin Instagram Reel 9:16 — 20s marketing (hook → oferta → local → CTA)."""
...
OUT_DIR = Path("/opt/cursor/artifacts/zapclin-reel-01")
...
def render_frame(i: int) -> Image.Image:
    ...
    if 2.9 < t < 7.2:
        ...
        lines = ["Em São Luís", "resolve em", "8 MINUTOS"]
        ...
        for idx, (txt, col) in enumerate(zip(lines, colors)):
            ...
            y += sizes[idx] + 18
        pill = "Higienização profissional de capacetes"
        fp = font(FONT_REG, 34)
        if local_t > 1.1:
...
def main():
    out_mp4 = OUT_DIR / "zapclin-reel-01-8min-9x16.mp4"
    out_mp4_repo = REPO_OUT / "zapclin-reel-01-8min-9x16.mp4"
    caption_path = OUT_DIR / "zapclin-reel-01-legenda.txt"
    caption_repo = REPO_OUT / "zapclin-reel-01-legenda.txt"

    caption = """Capacete fedendo? Em São Luís resolve em 8 minutos. ⚡

Higienização rápida a partir de R$ 15 no Golden Shopping Calhau (Quiosque 01).
Remove odor, germes e deixa o capacete pronto enquanto você resolve sua rotina no shopping.

Não é só máquina — é processo profissional ZapClin.

📍 Calhau · Golden Shopping Calhau
🌐 https://www.zapclinslz.com/
WhatsApp: 98 98147-9616

Salva este Reel e manda pra aquele amigo que tá com o capacete pesado de cheiro 👇
Link na bio · chama no Zap.

#ZapClin #HigienizacaoDeCapacete #SaoLuis #Calhau #GoldenShoppingCalhau #MotoboySLZ #CapaceteMoto #Maranhao #HigienizacaoSaoLuis
"""
    ...
    poster.save(OUT_DIR / "zapclin-reel-01-poster.jpg", quality=92)
    poster.save(REPO_OUT / "zapclin-reel-01-poster.jpg", quality=92)
